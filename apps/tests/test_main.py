...
from apps.schemas.ai_schemas import AIProjectPlan, AIProjectPlanRegenerateResponse, GeneratedTask, RegeneratedTask
...
def _valid_regenerate_request_body():
    return {
        'generation_id': '11111111-1111-1111-1111-111111111111',
        'project_id': '22222222-2222-2222-2222-222222222222',
        'title': 'Build a support platform',
        'tasks_to_regenerate': [
            {'temporary_id': 't1', 'title': 'Do it', 'reviewer_comment': 'Add more detail.'},
        ],
    }


def test_project_plan_regenerate_endpoint_returns_revised_tasks():
    fake_result = AIProjectPlanRegenerateResponse(tasks=[RegeneratedTask(temporary_id='t1', title='Do it well')])
    with patch('apps.main.regenerate_project_plan_tasks', new=AsyncMock(return_value=fake_result)):
        response = client.post('/api/v1/project-plan-regenerate', json=_valid_regenerate_request_body())
    assert response.status_code == 200
    body = response.json()
    assert body['success'] is True
    assert body['data']['tasks'][0]['title'] == 'Do it well'


def test_project_plan_regenerate_endpoint_maps_transient_provider_error():
    with patch('apps.main.regenerate_project_plan_tasks', new=AsyncMock(side_effect=TransientProviderError('rate limited'))):
        response = client.post('/api/v1/project-plan-regenerate', json=_valid_regenerate_request_body())
    assert response.status_code == 503
    assert response.json()['success'] is False


def test_project_plan_regenerate_endpoint_requires_service_token_when_configured():
    with patch('apps.main.settings.SERVICE_AUTH_TOKEN', 'secret'):
        response = client.post('/api/v1/project-plan-regenerate', json=_valid_regenerate_request_body())
    assert response.status_code == 401


def test_project_plan_regenerate_endpoint_requires_at_least_one_task():
    body = _valid_regenerate_request_body()
    body['tasks_to_regenerate'] = []
    response = client.post('/api/v1/project-plan-regenerate', json=body)
